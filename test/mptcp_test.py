...
net = Mininet(cleanup=True, controller=lambda name: RemoteController(name, ip='192.168.1.239'),
...
#c0 = net.addController('c0', ip='192.168.1.239')
...
if capture_mptcp_headers:
    ...
    'starting capturing...',
    print
    h2.cmd('tshark -f "tcp" -i any -a duration:' + str(test_duration + 1) + ' -T pdml ' +
...
if test_bandwith:
    print
    'starting iperf server at', h2.IP()
    h2.cmd('iperf -s -i 0.5 > iperf_bandwith_server_log.txt & ')  # server

    print
    'starting iperf client at', h1.IP(), ', connect to ', h2.IP()
    ...
    print
    h1.cmd('cat iperf_bandwith_client_log.txt')

    print
    "\niperf server response:"
    print
    h2.cmd('cat iperf_bandwith_server_log.txt')
...
if capture_mptcp_headers:
    try:
        ...
        print
        "capturing ended, result (" + str(len(capture_data)) + "):"
        for i in capture_data:
            ...
            package_info = ("%.2f" % (float(
                packet_timestamp) - test_started_timestamp)) + "\t" + src_ip + ':' + src_port + "\t->\t" + dst_ip + ':' + dst_port + "\t"
        ...
        print
        "\npackets count stat:\n", ",\n".join(
            [(i + ' : ' + str(packets_ip_stat[i])) for i in sorted(packets_ip_stat.keys())])
